...
// Load meet/.env first (canonical for meet-specific keys), then fall
// back to the parent .env at the repo root for shared keys like
// PERSONAL_ZOOM_ROOM. The `override: false` keeps meet/.env wins.
...
// When PERSONAL_ZOOM_ROOM is set, every avatar-into-meeting request is
// rewritten to this URL — regardless of what the caller passed. Useful
// during dev so you can drop into the same room and talk to the avatars
// without remembering to paste the URL into the UI each time. Empty
// string disables the override.
const PERSONAL_ZOOM_ROOM = (process.env.PERSONAL_ZOOM_ROOM || "").trim();
if (PERSONAL_ZOOM_ROOM) {
  console.log(
    `[config] PERSONAL_ZOOM_ROOM override active — every /api/start and ` +
      `/api/join request will be redirected to: ${PERSONAL_ZOOM_ROOM}`
  );
}

function resolveMeetingUrl(requested, label) {
  if (!PERSONAL_ZOOM_ROOM) return { url: requested, overridden: false };
  if (requested && requested !== PERSONAL_ZOOM_ROOM) {
    console.log(
      `[${label}] PERSONAL_ZOOM_ROOM override: rewriting ${requested} → ${PERSONAL_ZOOM_ROOM}`
    );
  }
  return { url: PERSONAL_ZOOM_ROOM, overridden: true };
}
...
// Transcript log per session: sessionId → array of { id, ts, participantIdentity, text, final }.
// Capped at TRANSCRIPT_BUFFER_MAX so a long-running session can't OOM the server;
// when the cap is reached we evict the oldest. Each segment is upserted by id so
// interim → final updates from LiveKit don't duplicate rows.
...
app.post("/api/start", (req, res) => {
  ...
  const { meetingUrl: requestedMeetingUrl, avatarType, avatarId, botName, maxDuration, systemPrompt, meetingPassword: requestedPassword } = req.body;

  if (!requestedMeetingUrl && !PERSONAL_ZOOM_ROOM)
    return res.status(400).json({ error: "meetingUrl required" });
  if (!avatarId) return res.status(400).json({ error: "avatarId required" });

  const { url: meetingUrl, overridden } = resolveMeetingUrl(requestedMeetingUrl, "start");
  // If we're forcing PERSONAL_ZOOM_ROOM, drop any caller-supplied password
  // — Zoom URLs from the personal-room env carry their own ?pwd=… and
  // mixing in a stale password produces "wrong passcode" failures.
  const meetingPassword = overridden ? undefined : requestedPassword;
...
app.get("/api/sessions/:id/transcript", (req, res) => {
  const log = transcripts.get(req.params.id) || [];
  // Optional ?since=N — only return entries after the Nth one (cheap incremental polling).
  const since = Number(req.query.since || 0);
  const slice = since > 0 ? log.slice(since) : log;
  res.json({
    sessionId: req.params.id,
    total: log.length,
    returned: slice.length,
    nextSince: log.length,
    entries: slice,
  });
});
...
app.post("/api/join", async (req, res) => {
  const { meetingUrl: requestedMeetingUrl, livekitUrl, livekitToken, botName, meetingPassword: requestedPassword } = req.body;
  ...
  const { url: meetingUrl, overridden } = resolveMeetingUrl(requestedMeetingUrl, "join");
  const meetingPassword = overridden ? undefined : requestedPassword;
...
httpServer.on("upgrade", (request, socket, head) => {
  ...
  // Route: /ws/transcript/:sessionId    — Bot page pushes transcript segments here
  const recallMatch = pathname.match(/^\/ws\/recall-video\/([^/]+)$/);
  const relayMatch = pathname.match(/^\/ws\/video-relay\/([^/]+)$/);
  const transcriptMatch = pathname.match(/^\/ws\/transcript\/([^/]+)$/);

  if (recallMatch || relayMatch || transcriptMatch) {
...
function handleTranscriptConnection(ws, sessionId) {
  console.log(`[ws] Transcript stream opened for session ${sessionId.slice(0, 8)}`);
  if (!transcripts.has(sessionId)) transcripts.set(sessionId, []);
  const log = transcripts.get(sessionId);
  // Index by segment id so interim → final updates upsert in place rather than duplicate.
  const indexById = new Map(log.map((seg, i) => [seg.id, i]));
  ...
  ws.on("message", (data) => {
    ...
    const entry = {
      ...
      participantIdentity: msg.participantIdentity || "unknown",
      text: msg.text,
      final: !!msg.final,
    };
    ...
    if (existingIdx !== undefined) {
      ...
      log.push(entry);
      indexById.set(entry.id, log.length - 1);
      if (log.length > TRANSCRIPT_BUFFER_MAX) {
        // Evict oldest; rebuild index lazily by trimming both structures.
        const evicted = log.shift();
        indexById.delete(evicted.id);
        // Indices in indexById are now off-by-one; cheapest fix is to rebuild.
        indexById.clear();
        log.forEach((s, i) => indexById.set(s.id, i));
      }
    }
    if (entry.final) {
      console.log(
        `[transcript ${sessionId.slice(0, 8)}] ${entry.participantIdentity}: ${entry.text}`,
      );
    }
  });

  ws.on("close", () => {
    console.log(`[ws] Transcript stream closed for session ${sessionId.slice(0, 8)}`);
  });
}
...
async function stopSession(session) {
  ...
  // Clean up relay clients and speaker state. Transcripts are kept around
  // so the user can GET /api/sessions/:id/transcript after the call ends;
  // they fall out of memory only when the server restarts.
